...
- **Caught process and privacy gaps the agents missed.** That commits weren't being made along the way (switched to per-step, attributed commits); and that the assignment brief and the interviewer's profile must not be published — directing their removal from the working tree *and* git history before any push.
